...
def run_optical_flow(vid_item):
    ...
    cmd = os.path.join(df_path + 'build/extract_gpu')+' -f={} -x={} -y={} -i={} -b=20 -t=1 -d={} -s=1 -o={} -w={} -h={}'.format(
        quote(vid_path), quote(flow_x_path), quote(flow_y_path), quote(image_path), 0, out_format, new_size[0], new_size[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--df_path", type=str, default='/home/darwin.ttito/gittest/two-stream-pytorch/dense_flow/',
    ...
    parser.add_argument("--start", type=int, default=0, help='start of index interval')
    parser.add_argument("--end", type=int, default=None, help='end of index interval (not included)')
    ...
    if(args.end is None): end = len(vid_list)
    else: end = args.end

    vid_list = vid_list[args.start:end]
